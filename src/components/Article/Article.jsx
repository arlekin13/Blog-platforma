...
function Article() {
  ...
  if (loading) {
    return (
      <div style={{ textAlign: 'center', padding: '20px', fontSize: '20px' }}>Загрузка...</div>
    );
  }

  if (error) {
    return (
      <div style={{ textAlign: 'center', padding: '20px', fontSize: '20px' }}>Ошибка: {error}</div>
    );
  }
          ...
          {isAuthor && (
            <>
              <div className={styles.article__actions}>
                ...
                <Button type="primary" onClick={handleEdit} className={styles.editButton}>
                  Edit
                </Button>
              </div>
            </>
